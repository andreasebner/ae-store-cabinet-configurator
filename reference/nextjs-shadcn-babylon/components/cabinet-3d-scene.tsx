...
const SIDE_FACE_MAP: Record<Side, number> = {
  front: 0, back: 1, right: 2, left: 3, top: 4, bottom: 5,
};

// Babylon face indices for a box: 0=back, 1=front, 2=right, 3=left, 4=top, 5=bottom
...
export default function Cabinet3DScene() {
  ...
  const initScene = useCallback(async () => {
    ...
    // Click handling - pick face to switch side
  ...
  const createBox = useCallback((scene: any) => {
    ...
    const box = BABYLON.MeshBuilder.CreateBox('cabinet', {
      width: w, height: h, depth: d,
    }, scene);
    boxRef.current = box;

    // Create multi-material for each face
    const multiMat = new BABYLON.MultiMaterial('cabinetMulti', scene);
    const faceMats: any[] = [];

    const baseColor = new BABYLON.Color3(0.85, 0.88, 0.92); // Light steel blue
    const activeColor = new BABYLON.Color3(0.35, 0.56, 0.96); // Blue highlight

    // 6 faces: back(0), front(1), right(2), left(3), top(4), bottom(5)
    SIDES.forEach(side => {
      ...
      // Add element count indicator via emissive glow
    ...
    // Box indices: each face has 6 indices (2 triangles)
    // Order: back(0-5), front(6-11), right(12-17), left(18-23), top(24-29), bottom(30-35)
  ...
  // Update box on cabinet/side/elements change
  useEffect(() => {
    if (sceneRef.current && BABYLON) {
      createBox(sceneRef.current);
    }
  }, [createBox]);

  // Animate camera on side change
  useEffect(() => {
  ...
  return (
    <div className="relative w-full h-full min-h-[200px]">
      <canvas ref={canvasRef} className="w-full h-full outline-none rounded-lg" />
