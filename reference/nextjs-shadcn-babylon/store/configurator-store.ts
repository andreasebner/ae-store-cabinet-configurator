...
import {
  SIDES, ELEMENT_DEFAULTS, MAX_UNDO, snap, calcPrice,
} from '@/lib/constants';
...
export const useConfiguratorStore = create<ConfiguratorStore>((set, get) => ({
  ...
  addElement: (type, clickX, clickY, canvasW, canvasH) => {
    ...
    const newEl: PanelElement = {
      id: state.nextId, type, x, y, w, h,
      ...(type === 'hole' ? { diameter: defaults.diameter } : {}),
    };
  ...
  undo: () => set(state => {
    ...
    const undoStack = [...state.undoStack]; const restored = undoStack.pop()!;
  ...
  redo: () => set(state => {
    ...
    const redoStack = [...state.redoStack]; const restored = redoStack.pop()!;
  ...
  addToCart: () => set(state => {
    ...
    return { cartItems: state.cartItems + 1, toastMessage: `Cabinet added to cart (${totalEls} cutout${totalEls !== 1 ? 's' : ''})`, toastIcon: '🛒' };
  }),
  ...
  currentElements: () => { const state = get(); return state.sideElements[state.currentSide]; },
}));
