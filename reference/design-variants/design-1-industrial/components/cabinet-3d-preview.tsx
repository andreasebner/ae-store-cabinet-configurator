...
export default function Cabinet3DPreview() {
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#12141A] relative">
      {/* Toolbar */}
      <div className="flex items-center justify-between px-4 py-3">
        <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#4B5563] font-mono">3D Preview</span>
        <div className="flex gap-0.5 bg-[#1A1D23] rounded-sm p-0.5 border border-[#3A3F4A]">
          {VIEW_MODES.map(vm => (
            <button
              key={vm.key}
              onClick={() => handleViewMode(vm.key)}
              className={`px-2.5 py-1 rounded-sm text-[10px] font-bold font-mono tracking-wider transition-all
                ${viewMode === vm.key
                  ? 'bg-[#FF6B2C] text-white shadow-[0_0_8px_rgba(255,107,44,0.3)]'
                  : 'text-[#6B7280] hover:bg-[#2A2F38] hover:text-[#9DA3AE]'}`}
            >
              {vm.label}
            </button>
          ))}
        ...
        >
          {SIDES_DATA.map(({ side, className }) => (
            ...
            >
              {side === 'front' && (
                <>
                  <div className="absolute right-5 top-1/2 -translate-y-1/2 w-1 h-[60px] bg-gradient-to-b from-[#4B5563] to-[#6B7280] rounded-sm shadow-[1px_0_2px_rgba(0,0,0,0.3)]" />
                  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-[5px]">
                    {[0,1,2,3].map(i => <div key={i} className="w-8 h-0.5 bg-white/5 rounded-sm" />)}
                  </div>
                </>
              )}
      ...
      {/* Zoom Controls */}
      <div className="absolute bottom-[70px] right-4 flex flex-col bg-[#1A1D23] rounded-sm border border-[#3A3F4A] overflow-hidden z-10">
        <button onClick={() => setZoom(zoomLevel + 0.15)} className="w-8 h-8 flex items-center justify-center text-[#6B7280] hover:bg-[#2A2F38] hover:text-[#FF6B2C] text-[15px] font-mono transition-all border-b border-[#3A3F4A]">+</button>
        <button onClick={() => setZoom(zoomLevel - 0.15)} className="w-8 h-8 flex items-center justify-center text-[#6B7280] hover:bg-[#2A2F38] hover:text-[#FF6B2C] text-[15px] font-mono transition-all border-b border-[#3A3F4A]">&#8722;</button>
        <button onClick={() => { setZoom(1); handleSelectSide(currentSide); }} className="w-8 h-8 flex items-center justify-center text-[#6B7280] hover:bg-[#2A2F38] hover:text-[#FF6B2C] text-[15px] font-mono transition-all">&#8865;</button>
      </div>
    </div>
  );
}
