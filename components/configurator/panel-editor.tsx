...
export default function PanelEditor() {
  ...
  return (
    <div className="flex-1 overflow-auto bg-slate-50 flex items-center justify-center p-6">
      <div
        ref={panelRef}
        className="relative bg-white shadow-lg border-2 border-slate-200 rounded-sm grid-dots"
        style={{
          width: pw * zoomLevel,
          height: ph * zoomLevel,
        ...
        {els.map(el => (
          <div
            ...
            className={cn(
              ...
              elStyle(el.type),
              selectedElId === el.id && 'ring-2 ring-brand-500 ring-offset-1 ring-offset-white'
            )}
            style={{
              left: el.x * zoomLevel,
              top: el.y * zoomLevel,
          ...
          >
            {elIcon(el.type)}
            {selectedElId === el.id && (
              <div className="absolute -right-1 -bottom-1 w-2.5 h-2.5 bg-brand-500 rounded-sm cursor-se-resize border border-white" />
            )}
          </div>
        ))}
      </div>
